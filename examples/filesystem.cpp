...
//
//   src/io/os/                  — thin POSIX-call wrappers (open/read/write/...).
//   src/io/io.hpp        — safe read/write helpers around fd_t / i32 fds.
//   src/io/file.hpp      — io::cached_file<T>, an OWNING file handle bound to
//                          a string buffer T (default micron::string).
//   src/io/fsys.hpp      — free functions: exists, rename, copy, move,
//                          make, file_type_at.
//   src/io/filesystem.hpp — io::filesystem<modes>, an in-memory cache
...
// cleans up. It uses the high-level io::cached_file<> for the read/write and
// io:: free functions for the metadata operations.
//
// STL deltas:
...
main()
{
  ...
  // ================================================================
  // 2. io oneshot functions — quick checks without opening
  // ================================================================
  micron::io::println("-- 2. io metadata --");

  micron::io::path_t target("/tmp/micron_fs_example.txt");

  // exists / readable_at / writeable_at / executable_at all wrap access(2)
  micron::io::println("exists before = ", micron::io::exists(target));

  // ================================================================
  // 3. io::cached_file<T> — write, then read back
  // ----------------------------------------------------------------
  ...
  {
    micron::io::cached_file<micron::string> f(target.c_str(), micron::io::modes::readwritecreate);
    micron::string payload("hello from micron!\nsecond line\n");
    f = payload;     // operator=(const T&) writes the buffer to the fd
  }     // f closes here

  micron::io::println("exists after write = ", micron::io::exists(target));

  // Read the file back: open in read mode, load(), pull() the buffer.
  {
    micron::io::cached_file<micron::string> f(target.c_str(), micron::io::modes::read);
    f.load();
  ...
  // ================================================================
  // 4. io::copy / move / rename
  // ----------------------------------------------------------------
  // Convenience wrappers around posix::rename and a buffered
  // copy. io::move falls back to copy+unlink across filesystems.
  // ================================================================
  micron::io::println("-- 4. copy / rename --");

  micron::io::path_t copy_target("/tmp/micron_fs_example_copy.txt");

  micron::io::copy(target, copy_target);
  micron::io::println("copy exists = ", micron::io::exists(copy_target));

  micron::io::path_t renamed("/tmp/micron_fs_example_renamed.txt");
  micron::io::rename(copy_target, renamed);
  micron::io::println("renamed exists  = ", micron::io::exists(renamed));
  micron::io::println("old name exists = ", micron::io::exists(copy_target));

  // ================================================================
  // 5. Cleanup
  // ----------------------------------------------------------------
  micron::io::unlink(target);
  micron::io::unlink(renamed);
  micron::io::println("-- 5. done; temp files removed --");

  // ================================================================
  // 6. Beyond this example
  // ----------------------------------------------------------------
  //   src/io/filesystem.hpp  — io::filesystem<> in-memory file cache
