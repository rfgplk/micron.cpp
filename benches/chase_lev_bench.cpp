...
bench_pingpong(u64 n) noexcept
{
  ...
  return median(s, K_MEASUREMENTS);
}
...
bench_steal(u32 K, u64 depth, u64 owner_ops, owner_mode mode, const char *label) noexcept
{
  ...
  const u64 centi_o = static_cast<u64>(median(own_s, K_MEASUREMENTS) * 100.0 + 0.5);
  const u64 centi_r = static_cast<u64>(median(rate_s, K_MEASUREMENTS) * 100.0 + 0.5);
  ...
  micron::io::print("owner ", centi_o / 100, ".", (centi_o % 100 < 10 ? "0" : ""), centi_o % 100, " ns/op   steals ", centi_r / 100, ".",
                    (centi_r % 100 < 10 ? "0" : ""), centi_r % 100, " M/s   got=", static_cast<u64>(median(got_s, K_MEASUREMENTS) + 0.5),
                    "% lost=", static_cast<u64>(median(lost_s, K_MEASUREMENTS) + 0.5), "%\n");
}
...
main(void)
{
  ...
  micron::io::print("median of ", static_cast<u64>(K_MEASUREMENTS), " (warmup ", static_cast<u64>(WARMUP), ")\n\n");

  micron::io::print("[owner] single-threaded paths -- reproduce to ~2%, a >2% delta is real\n");
  row("owner ", "pingpong-d1", 5000000, bench_pingpong(5000000), "ns/pair");
  row("owner ", "deep-d8", 8ULL * 400000, bench_deep(8, 400000), "ns/op");
  row("owner ", "deep-d64", 64ULL * 60000, bench_deep(64, 60000), "ns/op");
  row("owner ", "deep-d512", 512ULL * 8000, bench_deep(512, 8000), "ns/op");
  row("owner ", "deep-d1000", 1000ULL * 4000, bench_deep(1000, 4000), "ns/op");

  micron::io::print("\n[probe] the literal cl_sched::__steal sweep over EMPTY victims\n");
  row("probe ", "sweep-v4", 4, bench_probe_strided<192>(4, 40000), "ns/victim");
  row("probe ", "sweep-v8", 8, bench_probe_strided<192>(8, 20000), "ns/victim");
  row("probe ", "sweep-v16", 16, bench_probe_strided<192>(16, 8000), "ns/victim");
  row("probe ", "sweep-v32", 32, bench_probe_strided<192>(32, 2500), "ns/victim");
  row("probe ", "sweep-v16-mix", 16, bench_probe_mixed(16, 200000), "ns/sweep");

  row("probe ", "stride256-v16", 16, bench_probe_strided<256>(16, 8000), "ns/victim");
  row("probe ", "stride8448-v16", 16, bench_probe_strided<8448>(16, 8000), "ns/victim");
