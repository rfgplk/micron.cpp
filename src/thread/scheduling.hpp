...
get_scheduler(void)
{
  ...
  if ( r == -1 )
    throw except::system_error("micron::scheduling unable to get scheduler");
...
public:
  ...
  scheduler_t(pid_t pid)
  {
    if ( posix::get_attrs(pid, properties, 0) == -1 )
      throw except::system_error("micron::scheduler_t unable to get scheduling attributes");
  ...
  load_scheduler(const pid_t pid)
  {
    ...
    if ( posix::set_attrs(pid, properties, 0x00) == -1 )
      throw except::system_error("micron::scheduling unable to set scheduler");
  }
  static bool
