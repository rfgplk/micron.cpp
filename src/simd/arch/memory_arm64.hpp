...
__memset_bulk(u8 *__restrict d, const u8 v, const u64 n) noexcept
{
  ...
  u8 *const e = d + n;
  for ( ; p + 64 <= e; p += 64 ) __bits::__block_set_quad_16(p, vv);
...
__attribute__((nonnull)) inline u8 *
__memset_bulk_zero(u8 *__restrict d, const u64 n) noexcept
{
  const u64 dczid = __bits::__read_dczid();
  if ( (dczid & 0x10u) == 0 && (dczid & 0xFu) == 4u ) {
    const uint8x16_t z = __bits::__broadcast_byte_16(0);
    __bits::__block_set_quad_16(d, z);
    u8 *p = reinterpret_cast<u8 *>((reinterpret_cast<uintptr_t>(d) + 64) & ~static_cast<uintptr_t>(63));
    u8 *const e = d + n;
    for ( ; p + 64 <= e; p += 64 ) __bits::__dc_zva_64(p);
    __bits::__block_set_quad_16(e - 64, z);
    return d;
  }
  return __memset_bulk(d, 0, n);
}
...
__memset_bulk_nt(u8 *__restrict d, const u8 v, const u64 n) noexcept
{
  ...
  u8 *p = reinterpret_cast<u8 *>((reinterpret_cast<uintptr_t>(d) + 16) & ~static_cast<uintptr_t>(15));
  u8 *const e = d + n;
  for ( ; p + 32 <= e; p += 32 ) __bits::__block_set_pair_16_nt(p, vv);
  __neon_sfence();
  __bits::__block_set_16(e - 32, vv);
  __bits::__block_set_16(e - 16, vv);
  return d;
...
__memcpy_bulk(u8 *__restrict d, const u8 *__restrict s, const u64 n) noexcept
{
  ...
  const u8 *ps = s + off;
  u64 rem = n - off;
  for ( ; rem >= 64; rem -= 64, pd += 64, ps += 64 ) __bits::__block_copy_64(pd, ps);
  __bits::__block_copy_pair_16(d + n - 64, s + n - 64);
  __bits::__block_copy_pair_16(d + n - 32, s + n - 32);
  return d;
}
...
__memcpy_bulk_nt(u8 *__restrict d, const u8 *__restrict s, const u64 n) noexcept
{
  ...
  const u8 *ps = s + off;
  u64 rem = n - off;
  for ( ; rem >= 64; rem -= 64, pd += 64, ps += 64 ) __bits::__block_copy_64_nt(pd, ps);
  __neon_sfence();
  __bits::__block_copy_pair_16(d + n - 64, s + n - 64);
  __bits::__block_copy_pair_16(d + n - 32, s + n - 32);
  return d;
...
__memmove_bulk_fwd(u8 *d, const u8 *s, const u64 n) noexcept
{
  ...
  const u8 *ps = s + off;
  u64 rem = n - off;
  for ( ; rem >= 64; rem -= 64, pd += 64, ps += 64 ) __bits::__block_move_64(pd, ps);
