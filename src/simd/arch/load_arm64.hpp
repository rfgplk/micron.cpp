...
{
  return vsetq_lane_f64(*ptr, vdupq_n_f64(0.0), 0);
}
...
{
  return vsetq_lane_f64(*ptr, dst, 0);
}
...
{
  return vsetq_lane_f64(*ptr, dst, 1);
}
...
{
  static_assert(micron::is_same_v<B, d128> && micron::is_same_v<M, d128>, "maskload_pd: B and M must be d128");
  const d128 loaded = vld1q_f64(ptr);
  const int64x2_t smask = vshrq_n_s64(vreinterpretq_s64_f64(mask), 63);
...
maskgather_pd_i32(d128 src, double *base, i128 vindex, d128 mask)
{
  ...
  vst1q_s32(idx, vindex);
  vst1q_f64(msrc, src);
  vst1q_s64(mk, vreinterpretq_s64_f64(mask));
  for ( int i = 0; i < 2; ++i ) {
    if ( mk[i] < 0 )
      __builtin_memcpy(&out[i], __neon_scale_ptr<scale>(base, idx[i]), 8);
    else
