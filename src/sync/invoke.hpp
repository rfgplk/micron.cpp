...
{
  if constexpr ( std::is_member_pointer_v<micron::remove_cvref_t<F>> )
...
template <class F, class... Args>
constexpr bool
    negate_invocable_impl<std::void_t<decltype(!micron::invoke(std::declval<F>(), std::declval<Args>()...))>, F, Args...>
    = true;
...
template <class F>
constexpr micron::not_fn_t<std::decay_t<F>>
not_fn(F &&f)
{
