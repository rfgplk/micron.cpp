...
read(int fd, P *buf, size_t cnt)
{
  ...
  return ::syscall(SYS_read, fd, reinterpret_cast<void *>(buf), cnt);
}
template <typename P>
auto
write(int fd, P *buf, size_t cnt)
{
  ...
  //  returning
  //        the number of bytes actually transferred.  (This is true on both 32-bit and 64-bit systems.)
  return ::syscall(SYS_write, fd, reinterpret_cast<void *>(buf), cnt);
}
...
open(const char *name, int flags)
{
  ...
  //  returning
  //        the number of bytes actually transferred.  (This is true on both 32-bit and 64-bit systems.)
  return ::syscall(SYS_open, name, flags, 0);
}
...
openat(int dirfd, const char *pth, unsigned int mode)
{
  ...
  //  returning
  //        the number of bytes actually transferred.  (This is true on both 32-bit and 64-bit systems.)
  return ::syscall(SYS_openat, dirfd, pth, mode);
}
...
creat(const char *pth, unsigned int mode)
{
  ...
  //  returning
  //        the number of bytes actually transferred.  (This is true on both 32-bit and 64-bit systems.)
  return ::syscall(SYS_creat, pth, mode);
}
...
{
  return ::syscall(SYS_chmod, name, mode);
}
...
{
  return ::syscall(SYS_chown, name, owner, group);
}
...
{
  return ::syscall(SYS_fchown, name, owner, group);
}

auto
fchownat(int dirfd, const char *name, uid_t owner, gid_t group, int flags)
{
  return ::syscall(SYS_fchown, dirfd, name, owner, group, flags);
}
...
{
  return ::syscall(SYS_lseek, fd, mode, offset, len);
}
...
{
  return ::syscall(SYS_rename, oldfd, oldpath, newfd, newpath);
}

auto
renameat2(int oldfd, const char *__restrict oldpath, int newfd, const char *__restrict newpath, int flags)
{
  return ::syscall(SYS_rename, oldfd, oldpath, newfd, newpath, flags);
}
...
{
  return ::syscall(SYS_faccessat2, dirfd, name, mode, flags);
}
};
};
