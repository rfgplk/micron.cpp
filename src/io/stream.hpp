...
struct __intercept_holder {
  ...
  {
    if ( fn ) fn(const_cast<void *>(static_cast<const void *>(ctx)), data, sz);
  }
...
template <int __sz = __buffer_size, int __chnk = __buffer_packet> class stream
{
  ...
  {
    if ( fd.has_error() || fd.closed() ) exc<except::io_error>(where);
  }

  void
  __intercept()
  {
    if ( __hook.valid() ) __hook.call(__buffer->begin(), static_cast<usize>(__size));
  }
...
public:
  ...
  operator<<(const T &in)
  {
    if ( in.empty() ) exc<except::io_error>("io::stream operator<<: input string is empty.");
  ...
  operator<<(const T &in)
  {
    if ( in.empty() ) exc<except::io_error>("io::stream operator<<: input container is empty.");
  ...
  append_byte(byte b)
  {
    if ( static_cast<usize>(__size) >= static_cast<usize>(__sz) ) exc<except::io_error>("io::stream::append_byte: buffer full.");
  ...
  operator>>(T &out)
  {
    ...
    for ( max_t i = 0; i < __size; ++i ) out.push_back(static_cast<typename T::value_type>(p[i]));
    clear();
    return *this;
  }
  ...
  {
    if ( __size == 0 ) return;
    if ( out.size() < static_cast<usize>(__size) ) out.resize(static_cast<usize>(__size));
  ...
  {
    if ( out.has_error() || out.closed() ) return { false, 0 };
    if ( __size == 0 ) return { true, 0 };
    usize buf_i = 0;
  ...
  {
    if ( i >= static_cast<usize>(__size) ) exc<except::runtime_error>("io::stream[]: index out of bounds.");
    return (*__buffer)[i];
  }
  ...
  {
    if ( static_cast<usize>(__size) != str.size() * sizeof(typename T::value_type) ) return false;
  ...
  clear_valid()
  {
    if ( __size ) micron::memset(__buffer->begin(), 0, static_cast<usize>(__size));
    __size = 0;
  }
...
public:
  ...
  operator<<(const T &in)
  {
    if ( in.empty() ) exc<except::io_error>("io::stream_view operator<<: input is empty.");
    __raw_append(reinterpret_cast<const byte *>(in.c_str()), in.size() * sizeof(typename T::value_type));
    __intercept();
    return *this;
  }
