//  Copyright (c) 2024- David Lucius Severus
//
//  Distributed under the Boost Software License, Version 1.0.
//  See accompanying file LICENSE_1_0.txt or copy at
//  http://www.boost.org/LICENSE_1_0.txt
#pragma once

#include "../../../__special/initializer_list"

#include "../../bits/impl.hpp"
#include "../../constants.hpp"
#include "../../generic.hpp"
#include "../../log.hpp"
#include "../../mk.hpp"
#include "../../sqrt.hpp"
#include "../../trig.hpp"

#include "../../../type_traits.hpp"
#include "../../../types.hpp"

#include "../../../except.hpp"

#include "vec3.hpp"
#include "vec4.hpp"

namespace micron
{

template<typename T = float>
  requires micron::is_floating_point_v<T>
struct alignas(micron::math::vec_align_v<T, 8>) vector_8 {
  T x, y, z, w, a, b, c, d;

  ~vector_8() = default;

  constexpr vector_8() : x(T{}), y(T{}), z(T{}), w(T{}), a(T{}), b(T{}), c(T{}), d(T{}) { }

  constexpr vector_8(T x_, T y_, T z_, T w_, T a_, T b_, T c_, T d_) : x(x_), y(y_), z(z_), w(w_), a(a_), b(b_), c(c_), d(d_) { }

  constexpr vector_8(const vector_8<T> &o) = default;
  constexpr vector_8(vector_8<T> &&o) = default;

  constexpr vector_8(const std::initializer_list<T> &o)
  {
    if ( o.size() != 8 ) exc<except::library_error>("vector_8(): initializer_list size isn't equal to 8");
    auto it = o.begin();
    x = *it++;
    y = *it++;
    z = *it++;
    w = *it++;
    a = *it++;
    b = *it++;
    c = *it++;
    d = *it++;
  }

  constexpr vector_8<T> &operator=(const vector_8<T> &o) = default;
  constexpr vector_8<T> &operator=(vector_8<T> &&o) = default;

#define __v8_binop(op)                                                                                                                     \
  constexpr vector_8<T> operator op(const vector_8<T> &o) const                                                                            \
  {                                                                                                                                        \
    return { x op o.x, y op o.y, z op o.z, w op o.w, a op o.a, b op o.b, c op o.c, d op o.d };                                             \
  }
  __v8_binop(+) __v8_binop(-) __v8_binop(*)
#undef __v8_binop

#define __v8_selfop(op)                                                                                                                    \
  constexpr vector_8<T> &operator op(const vector_8<T> &o)                                                                                 \
  {                                                                                                                                        \
    x op o.x;                                                                                                                              \
    y op o.y;                                                                                                                              \
    z op o.z;                                                                                                                              \
    w op o.w;                                                                                                                              \
    a op o.a;                                                                                                                              \
    b op o.b;                                                                                                                              \
    c op o.c;                                                                                                                              \
    d op o.d;                                                                                                                              \
    return *this;                                                                                                                          \
  }
      __v8_selfop(+=) __v8_selfop(-=)
#undef __v8_selfop

          constexpr vector_8<T> product(const vector_8<T> &o) const
  {
    return *this * o;
  }

  constexpr vector_8<T>
  operator*(T s) const
  {
    return { x * s, y * s, z * s, w * s, a * s, b * s, c * s, d * s };
  }

  constexpr vector_8<T> &
  operator*=(T s)
  {
    x *= s;
    y *= s;
    z *= s;
    w *= s;
    a *= s;
    b *= s;
    c *= s;
    d *= s;
    return *this;
  }

  friend constexpr vector_8<T>
  operator*(T s, const vector_8<T> &v)
  {
    return { v.x * s, v.y * s, v.z * s, v.w * s, v.a * s, v.b * s, v.c * s, v.d * s };
  }

  constexpr vector_8<T>
  operator/(T s) const
  {
    return { x / s, y / s, z / s, w / s, a / s, b / s, c / s, d / s };
  }

  constexpr vector_8<T> &
  operator/=(T s)
  {
    x /= s;
    y /= s;
    z /= s;
    w /= s;
    a /= s;
    b /= s;
    c /= s;
    d /= s;
    return *this;
  }

  constexpr vector_8<T>
  operator/(const vector_8<T> &v)
  {
    return { x / v.x, y / v.y, z / v.z, w / v.w, a / v.a, b / v.b, c / v.c, d / v.d };
  }

  friend constexpr vector_8<T>
  operator/(T s, const vector_8<T> &v)
  {
    return { s / v.x, s / v.y, s / v.z, s / v.w, s / v.a, s / v.b, s / v.c, s / v.d };
  }

  constexpr vector_8<T>
  operator+(T s) const
  {
    return { x + s, y + s, z + s, w + s, a + s, b + s, c + s, d + s };
  }

  constexpr vector_8<T>
  operator-(T s) const
  {
    return { x - s, y - s, z - s, w - s, a - s, b - s, c - s, d - s };
  }

  friend constexpr vector_8<T>
  operator+(T s, const vector_8<T> &v)
  {
    return { s + v.x, s + v.y, s + v.z, s + v.w, s + v.a, s + v.b, s + v.c, s + v.d };
  }

  friend constexpr vector_8<T>
  operator-(T s, const vector_8<T> &v)
  {
    return { s - v.x, s - v.y, s - v.z, s - v.w, s - v.a, s - v.b, s - v.c, s - v.d };
  }

  constexpr vector_8<T>
  operator-() const
  {
    return { -x, -y, -z, -w, -a, -b, -c, -d };
  }

  constexpr T
  dot(const vector_8<T> &o) const
  {
    return x * o.x + y * o.y + z * o.z + w * o.w + a * o.a + b * o.b + c * o.c + d * o.d;
  }

  constexpr T
  squared_norm() const
  {
    return x * x + y * y + z * z + w * w + a * a + b * b + c * c + d * d;
  }

  constexpr T
  magnitude() const
  {
    return math::fsqrt(squared_norm());
  }

  constexpr T
  l1_norm() const
  {
    return math::fabs(x) + math::fabs(y) + math::fabs(z) + math::fabs(w) + math::fabs(a) + math::fabs(b) + math::fabs(c) + math::fabs(d);
  }

  constexpr T
  linf_norm() const
  {
    T lo = math::fabsmax(math::fabsmax(math::fabs(x), math::fabs(y)), math::fabsmax(math::fabs(z), math::fabs(w)));
    T hi = math::fabsmax(math::fabsmax(math::fabs(a), math::fabs(b)), math::fabsmax(math::fabs(c), math::fabs(d)));
    return math::fabsmax(lo, hi);
  }

  constexpr T
  lp_norm(T p) const
  {
    return math::pow(math::pow(math::fabs(x), p) + math::pow(math::fabs(y), p) + math::pow(math::fabs(z), p) + math::pow(math::fabs(w), p)
                         + math::pow(math::fabs(a), p) + math::pow(math::fabs(b), p) + math::pow(math::fabs(c), p)
                         + math::pow(math::fabs(d), p),
                     T{ 1 } / p);
  }

  constexpr T
  distance(const vector_8<T> &v) const
  {
    return (*this - v).magnitude();
  }

  constexpr T
  squared_distance(const vector_8<T> &v) const
  {
    return (*this - v).squared_norm();
  }

  constexpr vector_8<T>
  normalized() const
  {
    T n2 = squared_norm();
    if ( n2 <= math::default_eps<T>() ) return { T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 } };
    T inv = math::frsqrt(n2);
    return { x * inv, y * inv, z * inv, w * inv, a * inv, b * inv, c * inv, d * inv };
  }

  constexpr vector_8<T> &
  normalize()
  {
    T n2 = squared_norm();
    if ( n2 <= math::default_eps<T>() ) return *this;
    T inv = math::frsqrt(n2);
    x *= inv;
    y *= inv;
    z *= inv;
    w *= inv;
    a *= inv;
    b *= inv;
    c *= inv;
    d *= inv;
    return *this;
  }

  constexpr bool
  is_normalized(T eps = math::default_eps<T>()) const
  {
    return math::fabs(squared_norm() - T{ 1 }) <= eps;
  }

  constexpr T
  cos_angle(const vector_8<T> &v) const
  {
    T denom = squared_norm() * v.squared_norm();
    if ( denom <= math::default_eps<T>() ) return T{ 0 };
    return dot(v) * math::frsqrt(denom);
  }

  constexpr T
  angle(const vector_8<T> &v) const
  {
    T denom = squared_norm() * v.squared_norm();
    if ( denom <= math::default_eps<T>() ) return T{ 0 };
    return math::acos(math::fclamp(dot(v) * math::frsqrt(denom), T{ -1 }, T{ 1 }));
  }

  constexpr T
  scalar_project(const vector_8<T> &v) const
  {
    T n2 = v.squared_norm();
    if ( n2 <= math::default_eps<T>() ) return T{ 0 };
    return dot(v) / math::fsqrt(n2);
  }

  constexpr vector_8<T>
  project_onto(const vector_8<T> &v) const
  {
    T n2 = v.squared_norm();
    if ( n2 <= math::default_eps<T>() ) return { T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 }, T{ 0 } };
    return v * (dot(v) / n2);
  }

  constexpr vector_8<T>
  reject_from(const vector_8<T> &v) const
  {
    return *this - project_onto(v);
  }

  constexpr vector_8<T>
  reflect(const vector_8<T> &normal) const
  {
    return *this - normal * (T{ 2 } * dot(normal));
  }

  constexpr vector_8<T>
  refract(const vector_8<T> &normal, T eta) const
  {
    T nd = dot(normal);
    T k = T{ 1 } - eta * eta * (T{ 1 } - nd * nd);
    if ( k < T{ 0 } ) return {};
    return *this * eta - normal * (eta * nd + math::fsqrt(k));
  }

#define __b8_unop(name, fn)                                                                                                                \
  constexpr vector_8<T> name() const { return { fn(x), fn(y), fn(z), fn(w), fn(a), fn(b), fn(c), fn(d) }; }

  __b8_unop(abs, math::fabs) __b8_unop(floor, math::ffloor) __b8_unop(ceil, math::fceil) __b8_unop(round, math::fround)
      __b8_unop(trunc, math::ftrunc) __b8_unop(frac, math::ffract) __b8_unop(sqrt, math::fsqrt) __b8_unop(rsqrt, math::frsqrt)
          __b8_unop(exp, math::exp) __b8_unop(exp2, math::exp2) __b8_unop(log, math::log) __b8_unop(log2, math::log2)
              __b8_unop(log10, math::log10) __b8_unop(sin, math::sin) __b8_unop(cos, math::cos) __b8_unop(tan, math::tan)
                  __b8_unop(asin, math::asin) __b8_unop(acos, math::acos) __b8_unop(atan, math::atan) __b8_unop(sinh, math::sinh)
                      __b8_unop(cosh, math::cosh) __b8_unop(tanh, math::tanh) __b8_unop(erf, math::erf) __b8_unop(erfc, math::erfc)
                          __b8_unop(gamma, math::tgamma)
#undef __b8_unop

                              constexpr vector_8<T> sign() const
  {
    return {
      static_cast<T>(math::isign(x)), static_cast<T>(math::isign(y)), static_cast<T>(math::isign(z)), static_cast<T>(math::isign(w)),
      static_cast<T>(math::isign(a)), static_cast<T>(math::isign(b)), static_cast<T>(math::isign(c)), static_cast<T>(math::isign(d))
    };
  }

  constexpr vector_8<T>
  saturate() const
  {
    return { math::fclamp(x, T{ 0 }, T{ 1 }), math::fclamp(y, T{ 0 }, T{ 1 }), math::fclamp(z, T{ 0 }, T{ 1 }),
             math::fclamp(w, T{ 0 }, T{ 1 }), math::fclamp(a, T{ 0 }, T{ 1 }), math::fclamp(b, T{ 0 }, T{ 1 }),
             math::fclamp(c, T{ 0 }, T{ 1 }), math::fclamp(d, T{ 0 }, T{ 1 }) };
  }

  constexpr vector_8<T>
  step(T edge) const
  {
    return { math::step(edge, x), math::step(edge, y), math::step(edge, z), math::step(edge, w),
             math::step(edge, a), math::step(edge, b), math::step(edge, c), math::step(edge, d) };
  }

  constexpr vector_8<T>
  smoothstep(T e0, T e1) const
  {
    return { math::smoothstep(e0, e1, x), math::smoothstep(e0, e1, y), math::smoothstep(e0, e1, z), math::smoothstep(e0, e1, w),
             math::smoothstep(e0, e1, a), math::smoothstep(e0, e1, b), math::smoothstep(e0, e1, c), math::smoothstep(e0, e1, d) };
  }

  constexpr vector_8<T>
  clamp(T lo, T hi) const
  {
    return { math::fclamp(x, lo, hi), math::fclamp(y, lo, hi), math::fclamp(z, lo, hi), math::fclamp(w, lo, hi),
             math::fclamp(a, lo, hi), math::fclamp(b, lo, hi), math::fclamp(c, lo, hi), math::fclamp(d, lo, hi) };
  }

  constexpr vector_8<T>
  clamp(const vector_8<T> &lo, const vector_8<T> &hi) const
  {
    return { math::fclamp(x, lo.x, hi.x), math::fclamp(y, lo.y, hi.y), math::fclamp(z, lo.z, hi.z), math::fclamp(w, lo.w, hi.w),
             math::fclamp(a, lo.a, hi.a), math::fclamp(b, lo.b, hi.b), math::fclamp(c, lo.c, hi.c), math::fclamp(d, lo.d, hi.d) };
  }

#define __v8_binop2(name, fn)                                                                                                              \
  constexpr vector_8<T> name(const vector_8<T> &v) const                                                                                   \
  {                                                                                                                                        \
    return { fn(x, v.x), fn(y, v.y), fn(z, v.z), fn(w, v.w), fn(a, v.a), fn(b, v.b), fn(c, v.c), fn(d, v.d) };                             \
  }
  __v8_binop2(min, math::fmin) __v8_binop2(max, math::fmax) __v8_binop2(fmin, math::fmin) __v8_binop2(fmax, math::fmax)
      __v8_binop2(pow, math::pow) __v8_binop2(atan2, math::atan2)
#undef __v8_binop2

          constexpr vector_8<T> pow(T s) const
  {
    return { math::pow(x, s), math::pow(y, s), math::pow(z, s), math::pow(w, s),
             math::pow(a, s), math::pow(b, s), math::pow(c, s), math::pow(d, s) };
  }

  friend constexpr vector_8<T>
  pow(T s, const vector_8<T> &v)
  {
    return { math::pow(s, v.x), math::pow(s, v.y), math::pow(s, v.z), math::pow(s, v.w),
             math::pow(s, v.a), math::pow(s, v.b), math::pow(s, v.c), math::pow(s, v.d) };
  }

  constexpr vector_8<T>
  atan2(T s) const
  {
    return { math::atan2(x, s), math::atan2(y, s), math::atan2(z, s), math::atan2(w, s),
             math::atan2(a, s), math::atan2(b, s), math::atan2(c, s), math::atan2(d, s) };
  }

  constexpr T
  sum() const
  {
    return x + y + z + w + a + b + c + d;
  }

  constexpr T
  mean() const
  {
    return sum() / T{ 8 };
  }

  constexpr T
  prod() const
  {
    return x * y * z * w * a * b * c * d;
  }

  constexpr T
  min_component() const
  {
    return math::fmin(math::fmin(math::fmin(x, y), math::fmin(z, w)), math::fmin(math::fmin(a, b), math::fmin(c, d)));
  }

  constexpr T
  max_component() const
  {
    return math::fmax(math::fmax(math::fmax(x, y), math::fmax(z, w)), math::fmax(math::fmax(a, b), math::fmax(c, d)));
  }

  constexpr int
  argmin() const
  {
    const T vs[8] = { x, y, z, w, a, b, c, d };
    int idx = 0;
    for ( int i = 1; i < 8; ++i )
      if ( vs[i] < vs[idx] ) idx = i;
    return idx;
  }

  constexpr int
  argmax() const
  {
    const T vs[8] = { x, y, z, w, a, b, c, d };
    int idx = 0;
    for ( int i = 1; i < 8; ++i )
      if ( vs[i] > vs[idx] ) idx = i;
    return idx;
  }

  constexpr bool
  all() const
  {
    return x != T{} && y != T{} && z != T{} && w != T{} && a != T{} && b != T{} && c != T{} && d != T{};
  }

  constexpr bool
  any() const
  {
    return x != T{} || y != T{} || z != T{} || w != T{} || a != T{} || b != T{} || c != T{} || d != T{};
  }

  constexpr bool
  operator==(const vector_8<T> &o) const
  {
    return x == o.x && y == o.y && z == o.z && w == o.w && a == o.a && b == o.b && c == o.c && d == o.d;
  }

  constexpr bool
  operator!=(const vector_8<T> &o) const
  {
    return !(*this == o);
  }

  constexpr bool
  almost_equal(const vector_8<T> &o, T eps = math::default_eps<T>()) const
  {
    return math::fabs(x - o.x) <= eps && math::fabs(y - o.y) <= eps && math::fabs(z - o.z) <= eps && math::fabs(w - o.w) <= eps
           && math::fabs(a - o.a) <= eps && math::fabs(b - o.b) <= eps && math::fabs(c - o.c) <= eps && math::fabs(d - o.d) <= eps;
  }

#define __v8_cmpop(op)                                                                                                                     \
  constexpr vector_8<T> operator op(const vector_8<T> &o) const                                                                            \
  {                                                                                                                                        \
    return { static_cast<T>(x op o.x), static_cast<T>(y op o.y), static_cast<T>(z op o.z), static_cast<T>(w op o.w),                       \
             static_cast<T>(a op o.a), static_cast<T>(b op o.b), static_cast<T>(c op o.c), static_cast<T>(d op o.d) };                     \
  }                                                                                                                                        \
  constexpr vector_8<T> operator op(T s) const                                                                                             \
  {                                                                                                                                        \
    return { static_cast<T>(x op s), static_cast<T>(y op s), static_cast<T>(z op s), static_cast<T>(w op s),                               \
             static_cast<T>(a op s), static_cast<T>(b op s), static_cast<T>(c op s), static_cast<T>(d op s) };                             \
  }                                                                                                                                        \
  friend constexpr vector_8<T> operator op(T s, const vector_8<T> &v)                                                                      \
  {                                                                                                                                        \
    return { static_cast<T>(s op v.x), static_cast<T>(s op v.y), static_cast<T>(s op v.z), static_cast<T>(s op v.w),                       \
             static_cast<T>(s op v.a), static_cast<T>(s op v.b), static_cast<T>(s op v.c), static_cast<T>(s op v.d) };                     \
  }
  __v8_cmpop(<) __v8_cmpop(<=) __v8_cmpop(>) __v8_cmpop(>=)
#undef __v8_cmpop

      constexpr vector_2<T> lo_xy() const
  {
    return { x, y };
  }

  constexpr vector_2<T>
  lo_zw() const
  {
    return { z, w };
  }

  constexpr vector_2<T>
  hi_ab() const
  {
    return { a, b };
  }

  constexpr vector_2<T>
  hi_cd() const
  {
    return { c, d };
  }

  constexpr vector_4<T>
  lo() const
  {
    return { x, y, z, w };
  }

  constexpr vector_4<T>
  hi() const
  {
    return { a, b, c, d };
  }

  constexpr vector_4<T>
  even() const
  {
    return { x, z, a, c };
  }

  constexpr vector_4<T>
  odd() const
  {
    return { y, w, b, d };
  }

  constexpr T
  s0() const
  {
    return x;
  }

  constexpr T
  s1() const
  {
    return y;
  }

  constexpr T
  s2() const
  {
    return z;
  }

  constexpr T
  s3() const
  {
    return w;
  }

  constexpr T
  s4() const
  {
    return a;
  }

  constexpr T
  s5() const
  {
    return b;
  }

  constexpr T
  s6() const
  {
    return c;
  }

  constexpr T
  s7() const
  {
    return d;
  }

  constexpr vector_8<T>
  fma(const vector_8<T> &v, const vector_8<T> &u) const
  {
    return vector_8<T>(math::ffma(x, v.x, u.x), math::ffma(y, v.y, u.y), math::ffma(z, v.z, u.z), math::ffma(w, v.w, u.w),
                       math::ffma(a, v.a, u.a), math::ffma(b, v.b, u.b), math::ffma(c, v.c, u.c), math::ffma(d, v.d, u.d));
  }

  constexpr vector_8<T>
  fma(T s, const vector_8<T> &u) const
  {
    return vector_8<T>(math::ffma(x, s, u.x), math::ffma(y, s, u.y), math::ffma(z, s, u.z), math::ffma(w, s, u.w), math::ffma(a, s, u.a),
                       math::ffma(b, s, u.b), math::ffma(c, s, u.c), math::ffma(d, s, u.d));
  }

  constexpr vector_8<T>
  mul_add(const vector_8<T> &v, const vector_8<T> &u) const
  {
    return fma(v, u);
  }

  constexpr vector_8<T>
  mul_add(T s, const vector_8<T> &u) const
  {
    return fma(s, u);
  }
};

}      // namespace micron
