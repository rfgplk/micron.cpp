...
// fdlibm s_erf.c far-tail constants for |x| in [1/0.35, 28)
inline constexpr f64 rb[7]
    = { -9.86494292470009928597e-03, -7.99283237680523006574e-01, -1.77579549177547519889e+01, -1.60636384855821916062e+02,
        -6.37566443368389627722e+02, -1.02509513161107724954e+03, -4.83519191608651397019e+02 };
inline constexpr f64 sb[8] = { 1.0,
...
erf_f64(f64 x) noexcept
{
  ...
  if ( ax < 0.84375 ) {
    ...
    f64 s = qq[0] + z * (qq[1] + z * (qq[2] + z * (qq[3] + z * (qq[4] + z * qq[5]))));
    return x + x * (r / s);
  }
  if ( ax < 1.25 ) {
    f64 s = ax - 1.0;
    f64 P = pa[0] + s * (pa[1] + s * (pa[2] + s * (pa[3] + s * (pa[4] + s * (pa[5] + s * pa[6])))));
    f64 Q = qa[0] + s * (qa[1] + s * (qa[2] + s * (qa[3] + s * (qa[4] + s * (qa[5] + s * qa[6])))));
    f64 r = erx + P / Q;
  ...
  if ( ax < 2.857142857142857 ) {
    ...
    S = sa[0] + s * (sa[1] + s * (sa[2] + s * (sa[3] + s * (sa[4] + s * (sa[5] + s * (sa[6] + s * (sa[7] + s * sa[8])))))));
  } else {
    R = rb[0] + s * (rb[1] + s * (rb[2] + s * (rb[3] + s * (rb[4] + s * (rb[5] + s * rb[6])))));
    S = sb[0] + s * (sb[1] + s * (sb[2] + s * (sb[3] + s * (sb[4] + s * (sb[5] + s * (sb[6] + s * sb[7]))))));
  }
...
tgamma_f64(f64 x) noexcept
{
  ...
  if ( x < 0.5 ) {
    if ( x <= 0.0 && round_ns::floor<f64>(x) == x ) return ieee::qnan_v<f64>();
...
j1_small(f64 x) noexcept
{
  ...
  constexpr f64 C[16] = {
    ...
    -1.0 / 542861032610856960000.0,      // (-1)^13 / (13! * 14!)
    0.0,
    0.0,
  };
  f64 r = C[13];
