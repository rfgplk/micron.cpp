...
{
  simd::d128 a0 = simd::neon::splat_f64(0.0), a1 = a0, a2 = a0, a3 = a0;
  usize i = 0;
  for ( ; i + 8 <= count; i += 8 ) {
    a0 = simd::neon::add(a0, simd::neon::load_f64(reinterpret_cast<const double *>(values + i)));
    a1 = simd::neon::add(a1, simd::neon::load_f64(reinterpret_cast<const double *>(values + i + 2)));
    a2 = simd::neon::add(a2, simd::neon::load_f64(reinterpret_cast<const double *>(values + i + 4)));
    a3 = simd::neon::add(a3, simd::neon::load_f64(reinterpret_cast<const double *>(values + i + 6)));
  }
  f64 result = __horizontal(simd::neon::add(simd::neon::add(a0, a1), simd::neon::add(a2, a3)));
  for ( ; i < count; ++i ) result += values[i];
  return result;
}
...
weighted_sum_fast(const f64 *weights, const f64 *values, usize count) noexcept
{
  ...
  usize i = 0;
  for ( ; i + 8 <= count; i += 8 ) {
    a0 = simd::neon::fma_f64(a0, simd::neon::load_f64(reinterpret_cast<const double *>(weights + i)),
                             simd::neon::load_f64(reinterpret_cast<const double *>(values + i)));
    a1 = simd::neon::fma_f64(a1, simd::neon::load_f64(reinterpret_cast<const double *>(weights + i + 2)),
                             simd::neon::load_f64(reinterpret_cast<const double *>(values + i + 2)));
    a2 = simd::neon::fma_f64(a2, simd::neon::load_f64(reinterpret_cast<const double *>(weights + i + 4)),
                             simd::neon::load_f64(reinterpret_cast<const double *>(values + i + 4)));
    a3 = simd::neon::fma_f64(a3, simd::neon::load_f64(reinterpret_cast<const double *>(weights + i + 6)),
                             simd::neon::load_f64(reinterpret_cast<const double *>(values + i + 6)));
  }
  f64 result = __horizontal(simd::neon::add(simd::neon::add(a0, a1), simd::neon::add(a2, a3)));
  for ( ; i < count; ++i ) result += weights[i] * values[i];
  return result;
...
diff_uniform(const f64 *values, f64 *out, usize count, f64 dx) noexcept
{
  ...
  out[0] = (values[1] - values[0]) / dx;
  const simd::d128 scale = simd::neon::splat_f64(double(f64(0.5) / dx));
  usize i = 1;
  for ( ; i + 2 <= count - 1; i += 2 ) {
    const simd::d128 lo = simd::neon::load_f64(reinterpret_cast<const double *>(values + i - 1));
    const simd::d128 hi = simd::neon::load_f64(reinterpret_cast<const double *>(values + i + 1));
    simd::neon::store_f64(reinterpret_cast<double *>(out + i), simd::neon::mul(simd::neon::sub(hi, lo), scale));
  }
...
cum_trapezoid_uniform(const f64 *values, f64 *out, usize count, f64 dx) noexcept
{
  ...
  usize i = 1;
  for ( ; i + 2 <= count; i += 2 ) {
    simd::d128 value = simd::neon::mul(simd::neon::add(simd::neon::load_f64(reinterpret_cast<const double *>(values + i - 1)),
                                                       simd::neon::load_f64(reinterpret_cast<const double *>(values + i))),
                                       scale);
    value = simd::neon::add(value, vextq_f64(zero, value, 1));
    value = simd::neon::add(value, simd::neon::splat_f64(double(carry)));
    simd::neon::store_f64(reinterpret_cast<double *>(out + i), value);
    carry = f64(simd::neon::get_lane_f64<1>(value));
  }
  for ( ; i < count; ++i ) {
...
{
  const simd::d128 s = simd::neon::splat_f64(double(scale));
  const simd::d128 o = simd::neon::splat_f64(double(offset));
  usize i = 0;
  for ( ; i + 2 <= count; i += 2 ) {
    const simd::d128 value = simd::neon::load_f64(reinterpret_cast<const double *>(values + i));
    simd::neon::store_f64(reinterpret_cast<double *>(out + i), simd::neon::fma_f64(o, value, s));
  }
  for ( ; i < count; ++i ) out[i] = values[i] * scale + offset;
}
