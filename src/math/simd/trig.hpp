...
sin(simd::f256 x) noexcept
{
  ...
  const simd::f128 lo = _mm256_cvtpd_ps(sin(lo_d));
  const simd::f128 hi = _mm256_cvtpd_ps(sin(hi_d));
...
cos(simd::f256 x) noexcept
{
  const simd::d256 lo_d = _mm256_cvtps_pd(_mm256_castps256_ps128(x));
  const simd::d256 hi_d = _mm256_cvtps_pd(_mm256_extractf128_ps(x, 1));
  const simd::f128 lo = _mm256_cvtpd_ps(cos(lo_d));
  const simd::f128 hi = _mm256_cvtpd_ps(cos(hi_d));
  return _mm256_insertf128_ps(_mm256_castps128_ps256(lo), hi, 1);
}
...
{
  return _mm256_castpd256_pd128(sin(_mm256_castpd128_pd256(x)));
}

[[gnu::flatten]] inline simd::f128
sin(simd::f128 x) noexcept
{
  return _mm256_castps256_ps128(sin(_mm256_castps128_ps256(x)));
}

[[gnu::flatten]] inline simd::d128
cos(simd::d128 x) noexcept
{
  return _mm256_castpd256_pd128(cos(_mm256_castpd128_pd256(x)));
}

[[gnu::flatten]] inline simd::f128
cos(simd::f128 x) noexcept
{
  return _mm256_castps256_ps128(cos(_mm256_castps128_ps256(x)));
}
