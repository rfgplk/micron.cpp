...
{
using __graph_word_vector = u32 __attribute__((vector_size(16)));

inline void
word_union_arch(u64 *out, const u64 *a, const u64 *b, usize words) noexcept
{
  constexpr usize word_step = sizeof(__graph_word_vector) / sizeof(u64);
  usize i = 0;
  for ( ; i + word_step <= words; i += word_step ) {
