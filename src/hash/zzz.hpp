...
z(const u8 *__restrict data, usize sz, u64 *__restrict out)
{
  ...
  const __m256i mask64 = _mm256_set1_epi64x(0xFFFFFFFFFFFFULL);

  while ( ptr < end ) {
    __m256i block;
    usize remaining = end - ptr;
    if ( remaining >= 32 ) [[unlikely]] {
      block = _mm256_load_si256(reinterpret_cast<const __m256i *>(ptr));
      ptr += 32;
    } else {
      u8 tmp[32];
...
zz(const u8 *__restrict data, usize sz, u64 *__restrict out)
{
  ...
  while ( ptr < end ) {
    ...
    for ( u64 r = 0; r < __zzzrounds; ++r ) {
      ...
      U = _mm256_permute2x128_si256(S0, S0, 0x01);
      S0 = _mm256_subs_epi8(S0, U);
