...
public:
  ...
  template <typename... Args> vector(size_t n, Args... args) : contiguous_memory_no_copy<T>(this->create(n * sizeof(T)))
  {
    for ( size_t i = 0; i < n; i++ )
  ...
  operator[](size_t from, size_t to) const
  {
    ...
    if ( from >= to or from > contiguous_memory_no_copy<T>::capacity or to > contiguous_memory_no_copy<T>::capacity )
  ...
  {
    if ( i - begin() >= contiguous_memory_no_copy<T>::length )
  ...
  weld(vector<F> &&o)
  {
    if ( contiguous_memory_no_copy<T>::length + o.length >= contiguous_memory_no_copy<T>::capacity )
      reserve(contiguous_memory_no_copy<T>::capacity + o.max_size());
    __impl_copy(&(contiguous_memory_no_copy<T>::memory)[contiguous_memory_no_copy<T>::length], &o.memory[0], o.length);
    // micron::memcpy(&(contiguous_memory_no_copy<T>::memory)[contiguous_memory_no_copy<T>::length],
    // &o.memory[0],
    //                o.length);
    contiguous_memory_no_copy<T>::length += o.length;
  ...
  {
    micron::swap(contiguous_memory_no_copy<T>::memory, o.memory);
    micron::swap(contiguous_memory_no_copy<T>::length, o.length);
    micron::swap(contiguous_memory_no_copy<T>::capacity, o.capacity);
  }
  size_t
  max_size() const
  {
