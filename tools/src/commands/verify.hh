...
__dir_of(const mc::string &path)
{
  ...
  if ( itr == nullptr ) return mc::string("./");
  return path.substr(path.begin(), itr + 1);
}

inline mc::string
__resolve_include(const mc::string &raw_token, const mc::string &dir_hint, bool quoted, const mc::vector<string_type> &search_paths)
{
  ...
  if ( quoted ) {
    ...
    if ( mc::posix::exists(candidate) ) return candidate;
  }

  for ( const auto &base : search_paths ) {
    mc::string candidate(base.begin());
    candidate += "/";
    candidate += token;
    mc::fmt::replace_all(candidate, "//", "/");
    if ( mc::posix::exists(candidate) ) return candidate;
...
bool
verify_compileability(mc::posix::time_t last_time, const S &path, const mc::vector<string_type> &search_paths)
{
  ...
  if ( mtime > root_time ) return true;
  ...
  for ( auto &line : lines ) {
    ...
    if ( mc::fmt::starts_with(line, "#include \"") ) {
      ...
      mc::string resolved = __resolve_include(token, dir, true, search_paths);
      if ( resolved.empty() )
        continue;      // effectively system header, skip
                       // NOTE: system headers could technically update, but treat them as static for now

      if ( verify_compileability(root_time, resolved, search_paths) ) return true;
      ...
      mc::string resolved = __resolve_include(token, dir, false, search_paths);
      if ( resolved.empty() ) continue;      // LITERAL system header
