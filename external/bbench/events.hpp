...
__cache_cfg(u64 id, u64 op = PERF_COUNT_HW_CACHE_OP_READ, u64 result = PERF_COUNT_HW_CACHE_RESULT_ACCESS)
{
...
inline constexpr event_def known_events[] = {
  ...
  { "L1-dcache-load-misses", PERF_TYPE_HW_CACHE,
    __cache_cfg(PERF_COUNT_HW_CACHE_L1D, PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_MISS) },
  ...
  { "L1-dcache-prefetch-misses", PERF_TYPE_HW_CACHE,
    __cache_cfg(PERF_COUNT_HW_CACHE_L1D, PERF_COUNT_HW_CACHE_OP_PREFETCH, PERF_COUNT_HW_CACHE_RESULT_MISS) },
  { "L1-icache-loads", PERF_TYPE_HW_CACHE, __cache_cfg(PERF_COUNT_HW_CACHE_L1I) },
  { "L1-icache-load-misses", PERF_TYPE_HW_CACHE,
    __cache_cfg(PERF_COUNT_HW_CACHE_L1I, PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_MISS) },
  { "LLC-loads", PERF_TYPE_HW_CACHE, __cache_cfg(PERF_COUNT_HW_CACHE_LL) },
  { "LLC-load-misses", PERF_TYPE_HW_CACHE,
    __cache_cfg(PERF_COUNT_HW_CACHE_LL, PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_MISS) },
  ...
  { "LLC-store-misses", PERF_TYPE_HW_CACHE,
    __cache_cfg(PERF_COUNT_HW_CACHE_LL, PERF_COUNT_HW_CACHE_OP_WRITE, PERF_COUNT_HW_CACHE_RESULT_MISS) },
  { "dTLB-loads", PERF_TYPE_HW_CACHE, __cache_cfg(PERF_COUNT_HW_CACHE_DTLB) },
  { "dTLB-load-misses", PERF_TYPE_HW_CACHE,
    __cache_cfg(PERF_COUNT_HW_CACHE_DTLB, PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_MISS) },
  { "iTLB-loads", PERF_TYPE_HW_CACHE, __cache_cfg(PERF_COUNT_HW_CACHE_ITLB) },
  { "iTLB-load-misses", PERF_TYPE_HW_CACHE,
    __cache_cfg(PERF_COUNT_HW_CACHE_ITLB, PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_MISS) },
  { "branch-loads", PERF_TYPE_HW_CACHE, __cache_cfg(PERF_COUNT_HW_CACHE_BPU) },
  { "branch-load-misses", PERF_TYPE_HW_CACHE,
    __cache_cfg(PERF_COUNT_HW_CACHE_BPU, PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_MISS) },
  { "node-loads", PERF_TYPE_HW_CACHE, __cache_cfg(PERF_COUNT_HW_CACHE_NODE) },
  { "node-load-misses", PERF_TYPE_HW_CACHE,
    __cache_cfg(PERF_COUNT_HW_CACHE_NODE, PERF_COUNT_HW_CACHE_OP_READ, PERF_COUNT_HW_CACHE_RESULT_MISS) },
};
...
struct dynamic_event {
  ...
  dynamic_event(dynamic_event &&o) noexcept
      : e_fd(o.e_fd), last_errno(o.last_errno), attr(o.attr), pretty_name(o.pretty_name), valid(o.valid)
  {
    o.e_fd = -1;
    o.valid = false;
  }
  ...
  void
  configure(const event_def &def, bool excl_kernel = true)
  {
...
struct dynamic_event_group {
  ...
  dynamic_event_group(dynamic_event_group &&o) noexcept : events(o.events), count(o.count)
  {
    o.events = nullptr;
    o.count = 0;
  }
  ...
  set_inherit(bool on)
  {
    for ( usize i = 0; i < count; ++i ) events[i].set_inherit(on);
  }

  void
  set_pinned(bool on)
  {
    for ( usize i = 0; i < count; ++i ) events[i].set_pinned(on);
  }

  void
  set_enable_on_exec(bool on)
  {
    for ( usize i = 0; i < count; ++i ) events[i].set_enable_on_exec(on);
  }
  ...
  begin(void)
  {
    for ( usize i = 0; i < count; ++i ) events[i].begin();
  }

  void
  end(void)
  {
    for ( usize i = 0; i < count; ++i ) events[i].end();
  }
  ...
  for_each(F &&fn)
  {
    for ( usize i = 0; i < count; ++i ) fn(events[i].pretty_name, events[i].retrieve(), events[i].last_errno);
  }
};
