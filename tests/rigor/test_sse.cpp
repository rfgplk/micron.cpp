...
static void test_load_store()
{
  ...
  for (int i = 0; i < 16; ++i) { ibuf[i] = (char)(i + 1); oibuf[i] = 0; }
...
static void test_arith_fp()
{
  ...
  ms::storeu_f32(sb, sum);  require_true(sb[0] == 7.0f);
  ms::storeu_f32(sb, prod); require_true(sb[0] == 12.0f);
  end_test_case();
}
...
static void test_minmax()
{
  ...
  alignas(16) int sb[4];
  ms::storeu_i128((__m128i_u*)sb, mn); require_true(sb[0] == -5);
  ms::storeu_i128((__m128i_u*)sb, mx); require_true(sb[0] == 10);
  ...
  ms::storeu_i128((__m128i_u*)ub, u_mn); require_true(ub[0] == 0x40);
  ms::storeu_i128((__m128i_u*)ub, u_mx); require_true(ub[0] == 0xC0);
  end_test_case();
}
