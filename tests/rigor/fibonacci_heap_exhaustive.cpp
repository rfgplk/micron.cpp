...
main()
{
  ...
  test_case("pop returns elements in descending order (drives consolidate)");
  {
    micron::fibonacci_heap<int> h;
    int in[] = { 3, 1, 4, 1, 5, 9, 2, 6, 8, 7 };
  ...
  {
    micron::fibonacci_heap<int> a;
    for ( int i = 0; i < 6; i++ ) a.insert(int(i));
    micron::fibonacci_heap<int> b(micron::move(a));
  ...
  {
    micron::fibonacci_heap<int> dst;
    for ( int i = 0; i < 64; i++ ) dst.insert(int(i));
    micron::fibonacci_heap<int> src;
  ...
  test_case("lifetime: non-trivial elements balance ctor/dtor");
  {
    counted::live = 0;
    {
      micron::fibonacci_heap<counted> h;
  ...
  test_case("lifetime: move-assign over non-empty destroys old elements");
  {
    counted::live = 0;
    {
      micron::fibonacci_heap<counted> dst;
      for ( int i = 0; i < 40; i++ ) dst.insert(counted(i));
      micron::fibonacci_heap<counted> src;
  ...
  sb::print("=== ALL FIBONACCI_HEAP TESTS PASSED ===");
  return 1;
}
