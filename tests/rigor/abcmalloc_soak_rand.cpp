...
run(u64 rseed)
{
  ...
  sb::print("    SOAK_OPS=", static_cast<usize>(SOAK_OPS), " sweep_every=", static_cast<usize>(SOAK_SWEEP_EVERY),
            " frag_every=", static_cast<usize>(SOAK_FRAG_EVERY), " with_seed=", rseed);

  rng_t r = rng_t::from_seed(rseed);
  abctest::counts cnt;
...
main(void)
{
  ...
  auto rng = micron::math::rng::xoshiro256ss::from_seed(tp.as<micron::unit::nanoseconds>());
  for ( u64 i : mc::u64_range<0, 10>() )
    if ( u64 r = run(micron::math::rng::dist::uniform_int<u64>(rng, 0u, micron::numeric_limits<u64>::max())); r != 1 ) return r;
  return 0;
}
