...
test_construction()
{
...

  {
    ...
    assert(v.size() == 10);
    for ( size_t i = 0; i < v.size(); ++i ) assert(v[i] == 0);
  }

  {
    micron::vector<int> v(8, 42);
    assert(v.size() == 8);
    for ( auto &x : v ) assert(x == 42);
  }

  {
    micron::vector<int> v{ 1, 2, 3, 4, 5 };
    assert(v.size() == 5);
    for ( size_t i = 0; i < 5; ++i ) assert(v[i] == int(i + 1));
  }
}
...
test_resize_reserve()
{
  ...
  for ( size_t i = 50; i < 100; ++i ) assert(v[i] == 7);
}
...
test_churn()
{
  ...
  for ( int r = 0; r < 100; ++r ) {
    ...
    for ( int i = 0; i < 100; ++i ) v.insert(v.begin(), i);
  }
...
test_assign_clear_reuse()
{
  ...
  for ( int i = 0; i < 50; ++i ) assert(v[i] == i);
}
...
test_insert_ranges()
{
  ...
  for ( int i = 5; i < 15; ++i ) assert(v[i] == 99);
}
...
test_slice_consistency()
{
  ...
  for ( size_t i = 0; i < s.size(); ++i ) assert(s[i] == int(i + 10));
}
...
test_insert_sort_stress()
{
  ...
  for ( int i = 1000; i >= 0; --i ) v.insert_sort(i);
